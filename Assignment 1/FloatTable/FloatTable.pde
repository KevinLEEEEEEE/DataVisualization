...
void setup() {
  FloatTables data = new FloatTables("milk-tea-coffee.tsv");
  ...
  int yearMin = 1910;
  int yearMax = 2004;
  float dataMin = data.getTableMin();
  float dataMax = data.getTableMax();
  
  drawLine(data, 0, rowCount, yearMax, yearMin, dataMax, dataMin);
  drawLine(data, 2, rowCount, yearMax, yearMin, dataMax, dataMin);
  drawLine(data, 1, rowCount, yearMax, yearMin, dataMax, dataMin);
}

void drawLine(FloatTables data, int col, int rowCount, int yearMax, int yearMin, float dataMax, float dataMin) {
   beginShape();
   
  for(int row = 0; row < rowCount; row++) {
    if (data.isValid(row, col)) {
      int year = yearMin + row;;
      float value = data.getFloat(row, col);
      float x = map(year, yearMin, yearMax, 0, 960);
      float y = map(value, dataMin, dataMax, 20, 460);
